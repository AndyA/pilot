...
pi_affine *pi_transform_invert(pi_affine *out, const pi_affine *m) {
  ...
  out->m[0][0] = (m->m[1][1] * c5 - m->m[1][2] * c4 + m->m[1][3] * c3) * id;
  out->m[0][1] = (-m->m[0][1] * c5 + m->m[0][2] * c4 - m->m[0][3] * c3) * id;
  out->m[0][2] = (m->m[3][1] * s5 - m->m[3][2] * s4 + m->m[3][3] * s3) * id;
  out->m[0][3] = (-m->m[2][1] * s5 + m->m[2][2] * s4 - m->m[2][3] * s3) * id;

  out->m[1][0] = (-m->m[1][0] * c5 + m->m[1][2] * c2 - m->m[1][3] * c1) * id;
  out->m[1][1] = (m->m[0][0] * c5 - m->m[0][2] * c2 + m->m[0][3] * c1) * id;
  out->m[1][2] = (-m->m[3][0] * s5 + m->m[3][2] * s2 - m->m[3][3] * s1) * id;
  out->m[1][3] = (m->m[2][0] * s5 - m->m[2][2] * s2 + m->m[2][3] * s1) * id;

  out->m[2][0] = (m->m[1][0] * c4 - m->m[1][1] * c2 + m->m[1][3] * c0) * id;
  out->m[2][1] = (-m->m[0][0] * c4 + m->m[0][1] * c2 - m->m[0][3] * c0) * id;
  out->m[2][2] = (m->m[3][0] * s4 - m->m[3][1] * s2 + m->m[3][3] * s0) * id;
  out->m[2][3] = (-m->m[2][0] * s4 + m->m[2][1] * s2 - m->m[2][3] * s0) * id;

  out->m[3][0] = (-m->m[1][0] * c3 + m->m[1][1] * c1 - m->m[1][2] * c0) * id;
  out->m[3][1] = (m->m[0][0] * c3 - m->m[0][1] * c1 + m->m[0][2] * c0) * id;
  out->m[3][2] = (-m->m[3][0] * s3 + m->m[3][1] * s1 - m->m[3][2] * s0) * id;
  out->m[3][3] = (m->m[2][0] * s3 - m->m[2][1] * s1 + m->m[2][2] * s0) * id;
